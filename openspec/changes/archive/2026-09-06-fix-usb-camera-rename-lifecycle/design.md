...
- **Initial open failures are synchronous; runtime failures are asynchronous.** `Start` performs device listing, resolution, and the initial `openDevice` call before reporting success. If any step fails, it returns the error without launching a capture goroutine or leaving a relay active. After streaming begins, capture failures are emitted as `usb:error` events. The frontend handles either path by stopping and removing the failed camera display and showing an error notification; camera errors never close the application.
...
- [Risk] Waiting for `captureLoop` to exit in `Stop` may briefly block the UI thread if `ReadFrame` is stuck. → Mitigation: `Stop` first cancels the context and shuts down the HTTP server; capture checks cancellation between reads and the `EAGAIN` retry path remains responsive.
- [Risk] Opening the camera synchronously may delay the start request while the device API responds. → Mitigation: this delay is limited to initial startup and lets the frontend present an accurate failure without briefly showing an unusable stream.
